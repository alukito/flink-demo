...
import { isEventEnvelope, type DashboardMessage, type EventEnvelope, type MetricName, type WindowStat } from '../context/EventContext';
...
export function DashboardProvider({ children }: { children: ReactNode }) {
  ...
  const onMessage = useCallback((message: DashboardMessage) => {
    if (message.replay === true) return;
    if (isEventEnvelope(message)) {
      setEvents((current) => appendUniqueEvent(current, message));
    }
